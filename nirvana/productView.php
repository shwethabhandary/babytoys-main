...
<?php $arr = array(
    ...
    foreach ($arr as $item) {
      ...
      $result = mysqli_query($con,"SELECT * FROM mostvisitednimish WHERE id={$id}");
     if(mysqli_num_rows($result) === 0){
         $result2 = $con->query("INSERT INTO mostvisitednimish (id,name,image,cost,count) values ({$id},'{$data['name']}','{$data['image']}','{$data['cost']}',1)");
     }
     else{
         while($row = $result -> fetch_assoc()){
            $count =1+$row["count"];
            $sql = "UPDATE mostvisitednimish set count='{$count}' where id={$id}";
...
.rate > label:hover ~ input:checked ~ label {
  ...
  <body class="goto-here">
    ...
    <!-- END nav -->
    <?php
        $sqlreview = "SELECT * FROM review  where prod_id=$id and company='nimish'";
